    ...
    public Jwt decode(String token) throws JwtException {
        ...
        try {
           ...
           /* if (!ALLOWED_REALMS.contains(realm)) {
            ...
            if (!this.keycloakProperties.getRealms().stream().map(KeycloakProperties.RealmConfig::getRealm).toList().contains(realm) && !this.keycloakProperties.getDefaultRealm().equalsIgnoreCase(realm)) {
                throw new JwtException("Realm not allowed: " + realm);
            }
